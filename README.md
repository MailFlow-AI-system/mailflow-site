...
`SITE_URL` is public site configuration used to build canonical URLs. Local defaults are documented
in `.env.example`; `.env` is ignored and must not be committed. Never place passwords, API keys,
tokens, or other secrets in public environment variables, source files, fixtures, browser tests, or
logs.
